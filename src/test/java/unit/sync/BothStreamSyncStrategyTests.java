...
public class BothStreamSyncStrategyTests {
  ...
  @BeforeEach
  void setUp() {
    strategy = new BothStreamSyncStrategy(executor, conflictHandler);
  }
  ...
  void handle_shouldThrowIOException_whenExecutorFails() throws Exception {
    ...
    Mockito.doThrow(new IOException("I/O error"))
        ...
        .execute(
            Mockito.any(),
            Mockito.any(),
            Mockito.eq(FileType.REGULAR_FILE.getValue())
        );

    Assertions.assertThrows(
        IOException.class,
        () -> strategy.handle(snapshot, 0, left, right, null)
    );
  ...
  void handle_shouldThrowIOException_whenConflictHandlerFails() throws Exception {
    ...
    Mockito.doThrow(new IOException("I/O error"))
        ...
        .handle(snapshot, 0, left, right, null);
